...
public class EscobaMatchRound {
    ...
    private void checkEscobaEnMesa(Boolean isPlayerHumanDealer) {
        ...
        for(Card card : tableCards) {
            sum += card.getNumber(); // No tendria que sumar el valor?
        }
        if(sum == 15) {
    ...
    public void dealCards(boolean isPlayerHumanDealer) {
        ...
        if (!isPlayerHumanDealer) {
            ...
            do {
                Card humanCard = deck.takeCard();
                humanCards.add(humanCard);
                roundPlayerHuman.setHandCards(humanCards);
                System.out.println(humanCard + " para el humano.");

                Card appCard = deck.takeCard();
                appCards.add(appCard);
                roundPlayerApp.setHandCards(appCards);
                System.out.println(appCard + " para la máquina.");
            }
    ...
    private void solveSevenWinner() {
        ...
        if (sevenQuantity(roundPlayerHuman.getPersonalDeck()) > sevenQuantity(roundPlayerApp.getPersonalDeck())) {
            LetterByLetterPrinter.println("El jugador gana 1 punto por cantidad de 7.");
            roundPlayerHuman.setRoundPoints(roundPlayerHuman.getRoundPoints() + 1);
            if(sevenQuantity(roundPlayerHuman.getPersonalDeck()) == 4) {
                LetterByLetterPrinter.println("El jugador gana 1 punto extra por tener todos los 7.");
                roundPlayerHuman.setRoundPoints(roundPlayerHuman.getRoundPoints() + 1);
            }
        } else if (sevenQuantity(roundPlayerHuman.getPersonalDeck()) < sevenQuantity(roundPlayerApp.getPersonalDeck())) {
            LetterByLetterPrinter.println("La app gana 1 punto por cantidad de 7.");
            roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints() + 1);
            if(sevenQuantity(roundPlayerApp.getPersonalDeck()) == 4) {
                LetterByLetterPrinter.println("La app gana 1 punto extra por tener todos los 7.");
                roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints() + 1);
            }
        } else {
            LetterByLetterPrinter.println("No hay puntos por cantidad de 7. Ambos jugadores tienen la misma cantidad.");
        }
    ...
    private Boolean sevenOroCheck(List<Card> cards) {
        ...
        if (cards.contains(cardSevenOro)) {
            return true;
        }
        return false;
    }
