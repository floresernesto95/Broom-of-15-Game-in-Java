...
public class HumanRoundPlayer extends RoundPlayer {
    ...
    public void playTurn(List<Card> tableCards) {
        ...
        conjunto.add(selectedCard); //Añade la carta seleccionada al conjunto de las cartas de la mesa
        List<List<Card>> subconjuntosAux = obtenerSubconjuntos(conjunto); // subconjuntosAux tiene todas las posibles combinaciones de cartas
        subconjuntosAux.removeIf(c -> !c.contains(selectedCard)); // Remueve de subconjuntosAux todas las cartas que no tengan nuestra carta seleccionada
        subconjuntosAux.removeIf(c -> c.stream().mapToInt(Card::getValue).sum() != 15); // Remueve de subconjuntos todos los conjuntos cuyos valores no suman 15
        ...
        if(subconjuntos.isEmpty()) {
            ...
            if(subconjuntos.size() == 1) {
                ...
                takingCardsFromTable(tableCards, selectedCards); // Toma de la mesa las cartas con las que se formo 15
            } else {
    ...
    private Integer getCardToPlay() {
        ...
        do {
            ...
            try {
                rtaSelectCard = scanner.nextInt();
                if(rtaSelectCard > -1 && rtaSelectCard < 3) {
                    isValidRta = true;
                } else {
                    rtaSelectCard = -1;
